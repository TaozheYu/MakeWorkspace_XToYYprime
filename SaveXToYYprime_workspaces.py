...
path_fw = os.environ['CMSSW_BASE']+"/src/Make3DTemplate_XToYYprime/"
...
if __name__ == "__main__":
    ...
    for category in categories: 
      for systematic in systematics:
        ...
        if sample == "QCD_madgraph_pythia8" and systematic == "nominal":
           ...
           Convert_3Dhist_to_1Dhist(sample,category,"mjetsUp",     mj1_bins,mj2_bins,mjj_bins,hist3D_names,hist_covert3Dto1D)
           Convert_3Dhist_to_1Dhist(sample,category,"mjetsDown",   mj1_bins,mj2_bins,mjj_bins,hist3D_names,hist_covert3Dto1D)
           Convert_3Dhist_to_1Dhist(sample,category,"mjetsinvUp",  mj1_bins,mj2_bins,mjj_bins,hist3D_names,hist_covert3Dto1D)
           Convert_3Dhist_to_1Dhist(sample,category,"mjetsinvDown",mj1_bins,mj2_bins,mjj_bins,hist3D_names,hist_covert3Dto1D)

        #convert the 3D hist to 1D
        Convert_3Dhist_to_1Dhist(sample,category,systematic,mj1_bins,mj2_bins,mjj_bins,hist3D_names,hist_covert3Dto1D)

        '''
        for i in range(1, hist3D_names[sample][category][systematic].GetNbinsX() + 1):
          for j in range(1, hist3D_names[sample][category][systematic].GetNbinsY() + 1):
            for k in range(1, hist3D_names[sample][category][systematic].GetNbinsZ() + 1):
               content = hist3D_names[sample][category][systematic].GetBinContent(i, j, k)
               if content <= 0:
                  #continue
                  #print (content)
                  hist3D_names[sample][category][systematic].SetBinContent(i, j, k, 1e-5)
                  #print ("There is a bin less 0")
               else:
                  #print (content)
                  #hist3D_names[sample][category][systematic].SetBinContent(i, j, k, 1)
                  #print ("There is a bin less 0")
                  continue
        ...
        for i in range(1, hist3D_names[sample][category][systematic].GetNbinsX() + 1):
          for j in range(1, hist3D_names[sample][category][systematic].GetNbinsY() + 1):
            for k in range(1, hist3D_names[sample][category][systematic].GetNbinsZ() + 1):
               content = hist3D_names[sample][category][systematic].GetBinContent(i, j, k)
               error = hist3D_names[sample][category][systematic].GetBinError(i, j, k)
               if content < error:
                  #continue
                  #print (error)
                  hist3D_names[sample][category][systematic].SetBinError(i, j, k, content*0.9)
                  #print ("There is a bin error large than content")
               else:
                  #print (content)
                  #hist3D_names[sample][category][systematic].SetBinContent(i, j, k, 1)
                  #print ("There is a bin less 0")
  ...
  plot_sys(store_path,samples,categories,systematics_names,varname_list,hist1D_names) 
  ...
  for category in categories: 
    MakeWorkspace(category,samples,systematics,Roodatahist_covert3Dto1D)  
    WriteDatacard(category,Roodatahist_covert3Dto1D)
  ...
  #os.system("combineCards.py HP=datacard_HP.txt LP=datacard_LP.txt > datacard_merge.txt")
  #os.system("text2workspace.py datacard_merge.txt -o workspace.root")
  #os.system("text2workspace.py datacard_rest.txt -o workspace.root")
  #os.system("combine -M AsymptoticLimits workspace.root -t -1 > combine_result.txt")
  #os.system("combine -M FitDiagnostics workspace.root --saveShapes --saveWithUncertainties --cminDefaultMinimizerStrategy 0 --ignoreCovWarning -n postfit ")
